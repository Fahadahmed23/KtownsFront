...
class MReservation extends AdminController {
    ...
    public function millenium_index() {
        $Q = \DB::table('bookings');
        $this->data['recordsTotal'] = count($Q->get());
        return view('admin.reservation.millenium.millenium-index', $this->data);
    }

    public function millenium_reservation_list() {
        $status = -1;
        ...
        $query = \DB::table('bookings')->select(['booking_hotels.RoomNumber', 'bookings.BookingID', 'bookings.FirstName', 'bookings.LastName', 'booking_hotels.SellingPrice', 'bookings.Discount', 'bookings.PromoDiscount', 'bookings.TotalAmount', 'miscellaneous.BookingID AS MBookingID', 'bookings.TotalAmount', 'bookings.CheckingStatus AS Checking', 'bookings.Discount', 'hotels.SellingPrice AS Actual',
                ...
                ->where('booking_hotels.HotelID', '14')
    ...
    public function millenium_add() {
        $room_number = \DB::table('rooms')->select(DB::raw('rooms.*'))->where('HotelID', 14)->where('Status', 1)->get();

        $allocate_rooms = \DB::table('rooms')->select(DB::raw('rooms.*'))
                        ->leftjoin('booking_hotels', 'booking_hotels.RoomNumber', '=', 'rooms.RoomNumber')
                        ->where('rooms.HotelID', 14)->where('rooms.Status', 1)->whereRaw('CURDATE() between booking_hotels.CheckInDate and booking_hotels.CheckOutDate')->get();
        $rooms = array();
        ...
        $this->data['mRooms'] = 1;
        return view('admin.reservation.millenium.millenium-add', $this->data);
    }
    ...
    public function millenium_save() {
        $details = $this->get_hotel_details(14);
...
//                    return redirect()->back()->withInput()->with('error', 'Checkin date must be less than checkout date');
            ...
            if (\Input::get('CheckingStatus') == 1) {
                ...
                if ($currentDate == $checkInDate) {
                    ...
                    return redirect('admin/millenium/reservation')->with('success', "Reservation Added Successfully");
                } else {
                    return redirect('admin/millenium/reservation')->with('success', "Checkin date and System date doesn't match");
                }
            } else {
    ...
    public function millenium_edit($id) {
        $room_number = \DB::table('rooms')->select(DB::raw('rooms.*'))->where('HotelID', 14)->where('Status', 1)->get();
        $this->data['rooms_number'] = $room_number;
        ...
        $single_room = \DB::table('rooms')->select(DB::raw('rooms.*'))
                        ->leftjoin('booking_hotels', 'booking_hotels.RoomNumber', '=', 'rooms.RoomNumber')
                        ->where('rooms.HotelID', 14)->where('rooms.Status', 1)->where('booking_hotels.BookingID', $id)->get();
        ...
        return view('admin.reservation.millenium.millenium-edit', $this->data);
    }

    public function millenium_update($id) {
        $details = $this->get_hotel_details(14);
        $messages['required'] = 'Please enter :attribute.';
        $messages['min'] = 'Please select :attribute.';
        $error = false;
        ...
        if ($v->fails() || $error) {
            ...
            if ($myDiff > 0) {
                ...
                //echo json_encode(['error' => true, 'message' => "Checkin date must be less than checkout date"]);
                return redirect('admin/millenium/reservation/' . $id)->with('success', "Checkin date must be less than Checkout date");
//                    return redirect()->back()->withInput()->with('error', 'Checkin date must be less than checkout date');
            }
            if (\Session::get('AdminID') == 4) {
                $Admin = 127;
            ...
            if (\Input::get('CheckingStatus') == 0) {
                ...
                return redirect('admin/millenium/reservation')->with('success', "Reservation Updated Successfully");
                ...
                if ($currentDate == isset($bookings[0]->currentDate)) {
                    ...
                    return redirect('admin/millenium/reservation/' . $id)->with('success', "Checkout date and System date are same not update checkout date.");
                    ...
                    return redirect('admin/millenium/reservation')->with('success', "Checkout Date Updated Successfully");
                } else {
                    return redirect('admin/millenium/reservation/' . $id)->with('success', "Checkin date and System date doesn't match");
                }
            } else {
                return redirect('admin/millenium/reservation')->with('success', "Can't Update Because guest is checkout");
            }
        }
    }

    public function millenium_misc_add($id) {
        $this->data['miscid'] = $id;
        return view('admin.reservation.millenium.millenium-misc-add', $this->data);
    }

    public function millenium_misc_save($id) {
    ...
    public function millenium_misc_edit($id) {
        $this->data['miscid'] = $id;
        $this->data['miscellaneous'] = \DB::table('miscellaneous')->select(DB::raw('miscellaneous.*'))->where('BookingID', $id)->get();
        return view('admin.reservation.millenium.millenium-misc-edit', $this->data);
    }

    public function millenium_misc_update($id) {
        ...
        if ($bookings[0]->CheckingStatus == 1) {
            ...
            return redirect('admin/millenium/reservation')->with('success', "Miscellaneous Added Successfully");
        } else {
            return redirect('admin/millenium/reservation')->with('success', "Can't Update Because guest is checkout");
        }
    }

    public function millenium_create_pdf($id) {
    ...
    public function millenium_miscellaneous_create_pdf($id) {
    ...
    public function millenium_nightaudit_create_pdf() {
        $currentDate = date("Y-m-d");
    ...
    public function millenium_checkout_payment($id) {
        $this->data['miscid'] = $id;
        ...
        $this->data['miscellaneous'] = DB::table('miscellaneous')->select(DB::raw('Sum(miscellaneous.Amount) AS TotalAmount'))->where('BookingID', $id)->get();
        return view('admin.reservation.millenium.millenium-checkout-payment', $this->data);
    }

    public function millenium_checkoutpayment_update($id) {
        $messages['required'] = 'Please enter :attribute.';
        $messages['min'] = 'Please select :attribute.';
        $error = false;
        $msg = "";
        ...
        if ($v->fails() || $error) {
            ...
            return redirect('admin/millenium/reservation')->with('success', "Checkout Added Successfully");
        }
    }
    
    public function millenium_dashboard() {
        $currentDate = date("Y-m-d");
        ...
        $this->data['no_room'] = DB::table('rooms')->where('rooms.HotelID', 14)->count();
        ...
        return view('admin.reservation.millenium.millenium-dashboard', $this->data);
