       ...
       // dd(\Request::get('c'));
          ...
          //dd(Session::set('adults', $adults));
            ...
            if (\Request::has('daterange')) 
            {
                ...
                if (\Request::has('toDate')) {
                    $this->data['toDate']=\Request::get('toDate');
                    Session::set('toDate', $this->data['toDate']);
                } else {

                }
                if (\Request::has('fromDate')) {
                    $this->data['fromDate']=\Request::get('fromDate');
                    Session::set('fromDate', $this->data['fromDate']);
                    Session::set('daterange', $this->data['fromDate'] . ' - ' . $this->data['toDate']);
                } 
                
                
                // $this->data['adults']=\Request::get('adults');
                // $this->data['rooms']=$rooms;
                // dd('no daterange');
            }
        ...
        $pricingfilter= "";
         if (\Request::has('my_range')) {
            ...
            // dd(session()->all());
            $this->data['toDate']=Session::get('toDate');
            $this->data['fromDate']=Session::get('fromDate');
            $this->data['adults']=Session::get('adults');
            $this->data['rooms']=Session::get('rooms');
            // $this->data['children']=$children;
...
//        echo '<!--<pre>'.print_r($this->data['hotels'], true).'</pre>-->';
    ...
    public function get_hotel_detailsChild($SlugId) {
        $details = DB::table('hotels')
                ->select('hotels.HotelID', 'hotels.Slug', 'hotels.OriginalHotelName', 'hotels.OwnerName', 'hotels.Address', 'hotels.Address2', 'hotels.HotelName', 'Color', 'Border', 'NoOfGuests', 'AdultCharges', 'NoOfRooms', 'hotels.Description', 'hotels.MetaTitle', 'hotels.MetaDescription', 'hotels.MetaKeywords', 'hotels.Address', 'hotels.HotelTypeID', 'HotelTypeName', 'hotels.Image', 'hotels.Thumbnail', 'hotels.Rating', 'hotels.PartnerPrice', 'hotels.SellingPrice', 'hotels.AutoApprove', 'hotel_room_types.name as room_type_name', 'hotel_room_types.description as room_type_description', 'hotel_room_types.beds_information', 'hotel_room_types.beds_image', 'hotel_room_types.adults_sleep', 'hotel_room_types.children_sleep', 'hotel_room_types.Capacity', 'hotel_room_types.MaxAllowed', 'hotels.AdditionalGuestCharges')
                ->leftjoin('hotel_types', 'hotels.HotelTypeID', '=', 'hotel_types.HotelTypeID')
                ->leftjoin('hotel_room_types', 'hotels.hotel_room_type_id', '=', 'hotel_room_types.id')
...
//        \Session::forget('RoomsCart');
        ...
        if (empty(session()->get('toDate'))) {
            $today = date('Y-m-d');
            $tomorrow = date('m/d/Y', strtotime($today . " + 1 days"));
            session()->put('daterange', date('m/d/Y') . ' - ' . $tomorrow);
            session()->put('cityname', 'karachi');
            $tomorrow = date('Y-m-d', strtotime($today . " + 1 days"));
            session()->put('toDate', $tomorrow);
            session()->put('fromDate', date('Y-m-d'));
            session()->put('adults', 2);
            session()->put('rooms', 1);
        }
...
//echo  '<pre>'.print_r(\Session::get('RoomsCart'), true).'</pre>'; die;
        ...
        if (!empty($this->data['details'])) {
            ...
            $this->data['hotels'] = DB::table('hotels')
                ->select('HotelID', 'Slug', 'HotelName', 'Color', 'Border', 'hotels.HotelTypeID', 'HotelTypeName', 'hotels.Description', 'Rating', 'hotels.Image', 'hotels.Thumbnail', 'hotels.SellingPrice', 'hotels.MapImage')
                ->leftjoin('hotel_types', 'hotels.HotelTypeID', '=', 'hotel_types.HotelTypeID')
                ->where('hotels.Status', 1)
                ->limit(3)
                ->inRandomOrder()
...
//                echo $BookedRoom; die;
                ...
                if ($TotalGuests > $TotalMaxGuests && "a" == "b") {
...
//                    return redirect()->back()->withInput()->with('error', 'Maximum ' . $details->NoOfGuests . ' persons are allowed in each room');
                } else if ($TotalRooms > $details[0]->NoOfRooms && "a" == "b") {
                ...
                } else if ($TotalRooms > ($details[0]->NoOfRooms - $BookedRoom) && "a" == "b") {
                    echo json_encode(['error' => true, 'message' => 'There is no sufficient rooms in this guest house. Please select another guest house. Thanks']);
                } else {
